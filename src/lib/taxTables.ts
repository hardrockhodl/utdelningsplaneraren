...
export async function fetchTaxTable(
  ...
  try {
    ...
    const url = `${TAX_TABLE_API}?${query}&_limit=5000`;
    
    console.log('Fetching tax table:', { year, tableNumber, url });
    ...
    console.log('API first response:', {
      resultCount: data.resultCount,
      limit: data.limit,
      offset: data.offset,
      resultsInThisPage: data.results.length,
      hasNext: !!data.next
    });
    ...
    while (nextUrl && allResults.length < 10000) {
      ...
      console.log('Fetched page, total so far:', allResults.length);
    }
    
    console.log('✅ Total tax table entries fetched:', allResults.length);
    
    if (allResults.length > 0) {
      console.log('Income range:', {
        min: allResults[0]['inkomst fr.o.m.'],
        max: allResults[allResults.length - 1]['inkomst t.o.m.']
      });
    }
...
export function calculateTaxDeduction(
  ...
  if (!entry) {
    console.warn('No tax table entry found for salary:', grossSalary);
    if (grossSalary < toNumber(taxTable[0]['inkomst fr.o.m.'])) {
      return 0;
    }
    const lastEntry = taxTable[taxTable.length - 1];
    console.warn('Using last entry:', lastEntry);
...
} as const;export interface TaxTableEntry {
  'kolumn 1': string;
  'kolumn 2': string;
  'kolumn 3': string;
  'kolumn 4': string;
